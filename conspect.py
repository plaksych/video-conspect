...
def get_transcript_via_ytdlp(video_url):
    ...
    ydl_opts = {
        ...
        # --- ДОБАВЛЯЕМ ПРОКСИ ДЛЯ ОБХОДА БЛОКИРОВКИ ---
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            # extract_info теперь не должен зависнуть намертво из-за socket_timeout
            ...
            print(f"   [ОК] Успешно загружены субтитры (Язык: {lang})")
            return full_text.strip()
            
    except Exception as e:
        print(f"\n[ОШИБКА] Ошибка при работе yt-dlp: {e}")
        print("Если проблема в сети или блокировках YouTube, попробуйте запустить скрипт с включенным VPN.")
        return None
...
def generate_md_summary(text):
    ...
    prompt = (
        "Напиши подробный, но структурированный конспект (summary) лекции/видео на основе "
        "следующей текстовой расшифровки. Используй Markdown форматирование (заголовки, списки, таблицы). "
        "Убери вводные слова и воду, оставь только самую главную суть темы. Язык: русский.\n\n"
        f"Текст расшифровки:\n{text}"
    )
